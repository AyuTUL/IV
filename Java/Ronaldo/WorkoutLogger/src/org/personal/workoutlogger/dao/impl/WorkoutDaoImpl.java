...
public class WorkoutDaoImpl implements WorkoutDao {
    ...
    public List<Workout> getWorkoutsByUser(int uid) {
        ...
        try (Connection c = ConnectionFactory.getConnection(); PreparedStatement p = c.prepareStatement(sql)) {
            ...
            try (ResultSet r = p.executeQuery()) {
                while (r.next()) {
                    list.add(new Workout(r.getInt("id"), r.getInt("user_id"), r.getDate("date").toLocalDate(), r.getString("name"), r.getString("notes")));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void updateWorkout(Workout w, boolean isTrainer) {
        String sql = "UPDATE workouts SET name=?, date=?, notes=? WHERE id=? AND user_id=?";
        try (Connection c = ConnectionFactory.getConnection(); PreparedStatement p = c.prepareStatement(sql)) {
            p.setString(1, w.getName());
            p.setDate(2, Date.valueOf(w.getDate()));
            p.setString(3, w.getNotes());
            p.setInt(4, w.getId());
            p.setInt(5, w.getUserId());
            p.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try (Connection c = ConnectionFactory.getConnection()) {
            try {
                ...
                try (PreparedStatement p = c.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                    p.setInt(1, w.getUserId());
                    p.setString(2, w.getName());
                    p.setDate(3, java.sql.Date.valueOf(w.getDate()));
                    p.setString(4, w.getNotes());
                    p.executeUpdate();
                    int wid = -1;
                    try (ResultSet g = p.getGeneratedKeys()) {
    ...
    public void replaceItemsForWorkout(int workoutId, java.util.List<WorkoutItem> items){
