...
public class BodyMetricWindow extends JFrame {
    private final User user; private javax.swing.JComboBox<User> cmbUser = null;
    ...
    if ("TRAINER".equalsIgnoreCase(user.getRole())) {
        ...
        for (org.personal.workoutlogger.model.User uu : new org.personal.workoutlogger.dao.impl.UserDaoImpl().getAllUsers()) cmbUser.addItem(uu);
        JPanel north = new JPanel(new FlowLayout(FlowLayout.LEFT)); north.add(new JLabel("Select User:")); north.add(cmbUser); add(north, BorderLayout.NORTH);
    }
