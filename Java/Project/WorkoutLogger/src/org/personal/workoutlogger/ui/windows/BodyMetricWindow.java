...
public class BodyMetricWindow extends JFrame {
    ...
    private javax.swing.JComboBox<User> cmbUser = null;
    private final BodyMetricDao dao = new BodyMetricDaoImpl();
    private final JTable table = new JTable(
            new DefaultTableModel(new Object[] { "ID", "Date", "Weight", "BodyFat", "Height", "Age" }, 0));

    public BodyMetricWindow(User u) {
        super();
        GlobalUISettings.setupFrame(this, "Body Metrics");
        this.user = u;
    ...
    private void initUI() {
        ...
        buttons.setBackground(GlobalUISettings.WINDOW_BG);
    ...
    private void loadData() {
        ...
        for (BodyMetric bm : dao.findByUser(user.getId())) {
            m.addRow(new Object[] { bm.getId(), bm.getDate(), bm.getWeight(), bm.getBodyFat(), bm.getHeight(),
                    bm.getAge() });
        }
    }
    ...
    private void onDelete() {
        ...
        int id = (int) table.getValueAt(row, 0);
        if (JOptionPane.showConfirmDialog(this, "Delete selected record?", "Confirm",
