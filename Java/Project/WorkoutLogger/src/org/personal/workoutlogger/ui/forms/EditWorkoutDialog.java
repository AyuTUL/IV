...
public class EditWorkoutDialog extends JDialog {
    ...
    private void buildUI() {
        ...
        header.setBackground(new Color(220, 223, 228)); // Use global background color
        ...
        JButton btnAdd = GlobalUISettings.createStyledButton("Add Item");
        JButton btnEdit = GlobalUISettings.createStyledButton("Edit Item");
    ...
    private void editSelectedItemDialog() {
        ...
        if (selectIndex >= 0)
            exCmb.setSelectedIndex(selectIndex);

        JSpinner spSets = new JSpinner(
                new SpinnerNumberModel(((Number) itemsModel.getValueAt(row, 1)).intValue(), 1, 20, 1));
        JSpinner spReps = new JSpinner(
                new SpinnerNumberModel(((Number) itemsModel.getValueAt(row, 2)).intValue(), 1, 50, 1));
        JSpinner spWeight = new JSpinner(
                new SpinnerNumberModel(((Number) itemsModel.getValueAt(row, 3)).doubleValue(), 0.0, 1000.0, 1.0));
        JSpinner spRest = new JSpinner(
                new SpinnerNumberModel(((Number) itemsModel.getValueAt(row, 4)).intValue(), 0, 600, 5));
        ...
        int ok = JOptionPane.showConfirmDialog(this, p, "Edit Item", JOptionPane.OK_CANCEL_OPTION,
                JOptionPane.PLAIN_MESSAGE);
        if (ok == JOptionPane.OK_OPTION) {
            Exercise ex = (Exercise) exCmb.getSelectedItem();
            String name = ex != null ? ex.getName() : "";
    ...
    private void onSave() {
        try {
            ...
            JOptionPane.showMessageDialog(this, "Error: " + ex.getMessage(), "Update failed",
