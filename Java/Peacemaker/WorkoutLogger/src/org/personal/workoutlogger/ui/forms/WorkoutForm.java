...
public class WorkoutForm extends JDialog {
    ...
    public WorkoutForm(Frame owner, User user) {
        ...
        tfName.setText("");
        // Items table for multiple exercises
        itemsModel = new javax.swing.table.DefaultTableModel(new Object[]{"ExerciseId", "Exercise", "Sets", "Reps", "Weight", "Rest"}, 0);
    ...
    private void saveWorkout() {
        try {
            ...
            int workoutId = workoutDao.createWorkout(w);
            ComboItem ex = (ComboItem) cbExercise.getSelectedItem();
            if (ex != null && workoutId > 0) {
                WorkoutItem item = new WorkoutItem(0, workoutId, ex.id, (Integer) spSets.getValue(), (Integer) spReps.getValue(), (Double) spWeight.getValue(), (Integer) spRest.getValue());
