...
public class BodyMetrics extends JFrame {
    ...
    private final DefaultTableModel model = new DefaultTableModel(new Object[]{"ID","User","Date","Weight (kg)","Body Fat %","Notes"}, 0);
    ...
    private void initUI() {
        ...
        top.add(new JLabel("User:")); top.add(cmbUser);
        top.add(new JLabel("From (YYYY-MM-DD):")); top.add(txtFrom);
        top.add(new JLabel("To:")); top.add(txtTo);
    ...
    private void loadUsers() {
        try {
            ...
            if ("trainer".equalsIgnoreCase(loggedInUser.getRole())) {
                for (User u: userDao.findAll()) cmbUser.addItem(new UserItem(u.getId(), u.getUsername()));
            } else {
                cmbUser.addItem(new UserItem(loggedInUser.getId(), loggedInUser.getUsername()));
    ...
    private void loadMetrics() {
        model.setRowCount(0);
        try {
            UserItem sel = (UserItem) cmbUser.getSelectedItem();
            if (sel==null) return;
            LocalDate from = LocalDate.parse(txtFrom.getText().trim());
            LocalDate to = LocalDate.parse(txtTo.getText().trim());
            List<BodyMetric> list = metricDao.findByUserAndDateRange(sel.id, from, to);
            for (BodyMetric m : list) {
                model.addRow(new Object[]{ m.getId(), sel.name, m.getDate(), m.getWeightKg(), m.getBodyFat(), m.getNotes() });
            }
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Failed to load metrics: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void add(ActionEvent e) {
        ...
        if (JOptionPane.showConfirmDialog(this, fields, "Add Body Metric", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
            try {
    ...
    private void edit(ActionEvent e) {
        ...
        if (JOptionPane.showConfirmDialog(this, fields, "Edit Body Metric", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
            try {
                BodyMetric m = new BodyMetric();
                m.setId(id);
