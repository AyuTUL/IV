...
public class WorkoutSessionDaoDbImpl implements WorkoutSessionDao {
    ...
    public WorkoutSession create(WorkoutSession s) {
        String sql = "INSERT INTO sessions(user_id, session_date, notes) VALUES(?,?,?)";
    ...
    public void update(WorkoutSession s) {
        String sql = "UPDATE sessions SET user_id=?, session_date=?, notes=? WHERE id=?";
        try (Connection c = getConnection();
    ...
    public WorkoutSession findById(int id) {
        String sql = "SELECT id, user_id, session_date, notes FROM sessions WHERE id=?";
        try (Connection c = getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setInt(1, id);
    ...
    public List<WorkoutSession> findByUserAndDateRange(int userId, LocalDate from, LocalDate to) {
        StringBuilder sb = new StringBuilder("SELECT id, user_id, session_date, notes FROM sessions WHERE user_id=?");
